...
function toggleFirstConnexion() {
  ...
  if (firstTime.checked) {
    if (matriculeGroup) matriculeGroup.style.display = "block";
    if (newPasswordGroup) newPasswordGroup.style.display = "block";
    if (passwordGroup) passwordGroup.style.display = "none";
  } else {
    if (matriculeGroup) matriculeGroup.style.display = "none";
...
async function handleLogin(event) {
  ...
  try {
    const response = await fetch(`${getApiBase()}/profs/login`, {
      method: "POST",
    ...
    localStorage.setItem("role", data.role || "enseignant");
    window.location.href = "../profs/html/dashboard.html";
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", () => {
  document
    ...
    .addEventListener("change", toggleFirstConnexion);
